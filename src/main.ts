...
(async () => {
  ...
  const draggableAssets = [
   ...
   const targetAreas = [
    new TargetArea({ id: 0, x: app.screen.width * 0.52, y: app.screen.height * 0.18, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 1, x: app.screen.width * 0.66, y: app.screen.height * 0.15, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 2, x: app.screen.width * 0.76, y: app.screen.height * 0.18, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 3, x: app.screen.width * 0.86, y: app.screen.height * 0.14, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 4, x: app.screen.width * 0.52, y: app.screen.height * 0.58, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 5, x: app.screen.width * 0.7, y: app.screen.height * 0.46, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 6, x: app.screen.width * 0.78, y: app.screen.height * 0.38, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 7, x: app.screen.width * 0.68, y: app.screen.height * 0.66, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 8, x: app.screen.width * 0.76, y: app.screen.height * 0.66, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 9, x: app.screen.width * 0.88, y: app.screen.height * 0.66, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 10, x: app.screen.width * 0.58, y: app.screen.height * 0.78, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
    new TargetArea({ id: 11, x: app.screen.width * 0.82, y: app.screen.height * 0.86, size: Math.min(app.screen.width, app.screen.height) * 0.12 }),
  ];
        ...
        onDragEnd: (obj) => {
          ...
          for (const area of targetAreas) {
            if (area.isOverlapping(obj) && !area.isOccupied) {
              ...
              console.log(`${asset.filename} snapped to target ${area.id} - ${isCorrect ? 'CORRECT' : 'INCORRECT'}`);
              break;
            }
            area.setHighlight(false);
          }
          console.log(`${asset.filename} ${snapped ? 'snapped' : 'not snapped'}`);
          checkAllSnapped();
        },
      })
    )
  ...
  submitButton.on('pointerdown', () => {
    ...
    let correctCount = 0;
    targetAreas.forEach(area => {
      if (area.occupiedBy && (area.occupiedBy as DraggableObject).correctTargetId === area.id) {
        correctCount++;
